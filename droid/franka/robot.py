...
class FrankaRobot:
    ...
    #     target_position = self._max_gripper_width * (1 - command)
    #     curr_width = self._gripper.get_state().width

    #     if target_position < curr_width - 1e-2: #closing
    #         print(f"closing gripper. tw: {target_position}. cw: {curr_width}")
    #         self._gripper.grasp(speed=0.2, force=3.0, epsilon_inner=0.04, epsilon_outer=0.04)
    #         # self._gripper.goto(width=0, speed=0.5, force=3.0)
    #         print(f"do we ever get here?")
    #     elif target_position > curr_width + 1e-2: #opening
    #         print(f"opening gripper. tw: {target_position}. cw: {curr_width}")
    #         # self._gripper.stop()
    #         self._gripper.grasp(grasp_width=self._max_gripper_width, speed=0.2, force=3.0, epsilon_inner=0.04, epsilon_outer=0.04)
    #        # self._gripper.goto(width=self._max_gripper_width, speed=0.5, force=3.0)

        # print(f"updating gripper. tw: {target_position}. cw: {curr_width}")
        # self._gripper.goto(width=self._max_gripper_width * (1 - command), speed=0.5, force=0.1, blocking=blocking)
        # print(f"done updating gripper. tw: {target_position}. cw: {curr_width}")
    
    def update_gripper(self, command, velocity=True, blocking=False):
        # Map velocity -> position if needed (unchanged)
        if velocity:
            gripper_delta = self._ik_solver.gripper_velocity_to_delta(command)
            command = gripper_delta + self.get_gripper_position()

        # Clamp to [0, 1] as before
        command = float(np.clip(command, 0, 1))

        # Convert normalized [0,1] "closedness" to an absolute width
        target_width = self._max_gripper_width * (1.0 - command)

        # Read current width; if state call fails, fall back to target (safe no-op for opening test)
        try:
            curr_width = self._gripper.get_state().width
        except Exception:
            curr_width = target_width

        # We consider it "opening" if target width is larger than current width by a tiny epsilon
        opening = (target_width > curr_width + 1e-3)

        # Choose sensible dynamics: open faster, close a bit gentler; force high enough to hold when closing
        open_speed, close_speed = 0.7, 0.5
        close_force = 3.0
        open_force = 3.0  # force is ignored by some drivers in position mode; safe to pass

        # Ensure stop()->goto happens as a single critical section
        with getattr(self, "_gripper_lock", None) or contextlib.nullcontext():
            if opening:
                # **NEW**: cancel any active grasp/hold before commanding an open
                try:
                    self._gripper.stop()
                    log.warning(f"STOPPOPPPPP currently: {curr_width}. tw: {target_width}")
                    # optional tiny pause to let firmware transition out of grasp/hold
                    time.sleep(0.01)
                except grpc.RpcError:
                    log.warning(f"UMM ERR STOP curr: {curr_width}, tw: {target_width}")
                    pass

                # Now go to the wider width (release)
                try:
                    # while (curr_width + 1e-3 < target_width):
                    log.warning(f"ok now going to release bc opening curr: {curr_width}, tw: {target_width}")
                    self._gripper.goto(
                        width=target_width,
                        speed=open_speed,
                        force=open_force,
                ...
                # Closing path (unchanged behavior, but with stronger force & a bit slower)
                try:
                    log.warning(f"This is the unchanged closing path behavior. curr: {curr_width}, tw: {target_width}")
                    self._gripper.goto(
                        width=target_width,
                        speed=close_speed,
                        force=close_force,
